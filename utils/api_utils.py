...
from config import TIMEZONE, now_vn
...
class WeatherAPI:
    ...
    def get_weather(self, lat, lon):
        try:
            ...
            if not cw:
                logger.warning(f"Open-Meteo: empty current_weather for ({lat}, {lon})")
                return None

            current_hour_str = cw.get("time", "")
    ...
    def get_historical_weather(self, lat, lon, start_date, end_date):
        ...
        try:
            ...
            if not times:
                logger.warning(f"Open-Meteo Archive: no data for ({lat}, {lon})")
                return []
            ...
            logger.info(f"Fetched {len(records)} archive records ({start_date} → {end_date})")
            return records

        except Exception as e:
            logger.error(f"Open-Meteo Archive connection error: {e}")
            return []
